...
def KL_loss(mu_1, log_sigma_1, mu_2=np.float32(0.0), log_sigma_2=np.float32(0.0)):
    ...
    kl = tf.reduce_mean(tf.distributions.kl_divergence(dist_1, dist_2), axis=-1)
    kl = tf.multiply(kl, mask)
    loss = tf.reduce_mean(kl, axis=-1)

    return loss


def cross_entropy_sequence_loss(logits, labels):
    cross_entropy = tf.nn.sparse_softmax_cross_entropy_with_logits(logits=logits[:, :-1, :], labels=labels[:, :-1])
